import net from 'net';
import { encodeOSCMessage, OSCTcpClient } from './osc';

function readCString(buf: Buffer, off: number) {
  let end = off;
...
function decodeOSC(buf: Buffer) {
  ...
  for (let i = 1; i < types.length; i++) {
    ...
    if (t === 'f') { args.push(buf.readFloatBE(off)); off += 4; }
      ...
      const r = readCString(buf, off); args.push(r.str); off = r.next;
    } else {
